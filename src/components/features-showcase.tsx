      ...
      <div className="mx-auto max-w-7xl px-6">
        ...
        <div className="grid md:grid-cols-3 gap-5">
          ...
          >
            <div className="flex flex-col lg:flex-row gap-6 h-full">
              <div className="lg:w-[42%] shrink-0">
                ...
                <p className="text-sm text-[var(--color-text-muted)] leading-relaxed mb-4">
                  每条请求实时评估 30+ 供应商的延迟、错误率与每千 token 成本，按加权得分自动选优渠道。支持按模型能力、地域合规、响应格式分层路由——无需任何配置。
                </p>
                <div className="flex items-baseline gap-2 mb-2">
                  <span className="text-3xl font-bold font-mono text-gradient-gold" style={{ fontVariantNumeric: "tabular-nums" }}>&lt;100ms</span>
                  <span className="text-xs text-[var(--color-text-muted)]">路由决策 p50 延迟</span>
                </div>
                <p className="text-[11px] text-[var(--color-text-muted)]/70 font-mono">
                  自动 failover · key 轮转 · 限流退避
                </p>
              </div>
            ...
            <div className="absolute -bottom-12 -right-12 w-48 h-48 rounded-full bg-[var(--color-accent)] opacity-[0.04] blur-[70px] group-hover:opacity-[0.08] transition-opacity duration-700 pointer-events-none" />
          </motion.div>
          ...
          >
            ...
            <p className="text-sm text-[var(--color-text-muted)] leading-relaxed mb-4">
              DECIMAL(20,4) 精度钱包，SQL 原子事务保证每笔扣款幂等。充值、预授权、退款全覆盖；支付回调双重幂等防超卖。
            </p>
            <div className="flex-1 bg-[var(--background)]/60 rounded-xl p-4 border border-[var(--color-border)]">
              <BillingLedgerVisual />
            </div>
            <div className="mt-4 flex items-baseline gap-2">
              <span className="text-2xl font-bold font-mono text-gradient-gold" style={{ fontVariantNumeric: "tabular-nums" }}>¥0.0001</span>
              <span className="text-xs text-[var(--color-text-muted)]">最小计费粒度</span>
            </div>
            <div className="absolute -bottom-8 -right-8 w-32 h-32 rounded-full bg-[var(--color-accent)] opacity-[0.04] blur-[50px] group-hover:opacity-[0.08] transition-opacity duration-700 pointer-events-none" />
          </motion.div>
          ...
          >
            ...
            <p className="mt-3 text-xs text-[var(--color-text-muted)] font-mono">持久化上下文 · per-user 记忆空间 · REST + MCP 双协议</p>
          </motion.div>
          ...
          >
            ...
            <p className="text-sm text-[var(--color-text-muted)] leading-relaxed">
              额度告急、支付完成、任务结束——用户在网页、邮箱、移动端都能第一时间收到，你一行通知代码都不用写。
            </p>
            <p className="mt-3 text-xs text-[var(--color-text-muted)] font-mono">NATS 事件驱动 · WebSocket · SMTP · FCM</p>
          </motion.div>
          ...
          >
            <div className="flex flex-col md:flex-row gap-8 items-start">
              <div className="md:w-64 shrink-0">
                ...
                <p className="text-sm text-[var(--color-text-muted)] leading-relaxed mb-5">
                  每个请求的模型、延迟、token 用量与状态码全部追踪，OpenTelemetry 原生兼容，预配置仪表盘零运维负担。
                </p>
                <div className="space-y-3">
                  {([
                    { value: "78ms", label: "P50 延迟", color: "var(--color-accent)" },
                    { value: "0.02%", label: "错误率", color: "var(--color-success)" },
                    { value: "100%", label: "请求可追踪", color: "var(--color-text-primary)" },
                  ] as const).map((s) => (
                    <div key={s.label} className="flex items-baseline gap-2">
                      ...
                      <span className="text-[10px] text-[var(--color-text-muted)] uppercase tracking-wide font-mono">{s.label}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="absolute -bottom-12 -right-12 w-56 h-56 rounded-full bg-[var(--color-accent)] opacity-[0.03] blur-[90px] group-hover:opacity-[0.06] transition-opacity duration-700 pointer-events-none" />
          </motion.div>
            ...
            >
              ...
              <div className="flex items-center gap-3 sm:contents text-[10px] sm:text-xs pl-3.5 sm:pl-0">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <span
                    className={`font-bold ${
                      p.selected ? "text-[var(--color-accent)]" : "text-[var(--color-text-muted)]"
                    }`}
                  >
                    {p.score}
                  </span>
...
function ObserveWideVisual() {
  ...
  // accent = #FF5D1F — use rgba approximation for SVG fill (CSS vars not supported in SVG fill attr)
  const accentFill = "rgba(255,93,31,";
  ...
  return (
    <div>
      ...
      <svg viewBox="0 0 390 55" className="w-full" aria-hidden="true">
        {bars.map((h, i) => (
          <motion.rect
            key={i}
            x={i * 16 + 2}
            ...
            height={h * 0.65}
            rx="2"
            fill={h > 65 ? `${accentFill}0.35)` : `${accentFill}0.15)`}
...
function MemoryVisual() {
  // accent #FF5D1F, accent-2 #2D4A8A, success #1F7A4F
  return (
    <svg viewBox="0 0 140 80" className="w-full h-full max-w-[200px]" aria-hidden="true">
      {memoryDots.map((d, i) => (
        <motion.circle
          key={i}
          cx={d.x}
          cy={d.y}
          r={d.size}
          fill="rgba(255,93,31,0.3)"
          animate={{ opacity: [0.2, 0.6, 0.2], scale: [1, 1.2, 1] }}
          transition={{ duration: d.dur, delay: d.del, repeat: Infinity }}
        />
      ))}
      <circle cx="45" cy="35" r="18" fill="none" stroke="rgba(255,93,31,0.1)" strokeWidth="0.5" strokeDasharray="3 2" />
      <circle cx="85" cy="40" r="15" fill="none" stroke="rgba(45,74,138,0.1)" strokeWidth="0.5" strokeDasharray="3 2" />
      <circle cx="65" cy="55" r="12" fill="none" stroke="rgba(31,122,79,0.1)" strokeWidth="0.5" strokeDasharray="3 2" />
      <motion.g animate={{ opacity: [0, 1, 1, 0] }} transition={{ duration: 4, repeat: Infinity }}>
        <circle cx="110" cy="15" r="4" fill="rgba(31,122,79,0.2)" stroke="rgba(31,122,79,0.5)" strokeWidth="0.5" />
        <text x="110" y="17" textAnchor="middle" fill="#1F7A4F" fontSize="3.5">Q</text>
        <line x1="106" y1="18" x2="88" y2="32" stroke="#1F7A4F" strokeWidth="0.5" strokeDasharray="2 2" />
      </motion.g>
    </svg>
  );
}
  ...
  return (
    <svg viewBox="0 0 140 80" className="w-full h-full max-w-[200px]" aria-hidden="true">
      <motion.g animate={{ y: [0, -2, 0] }} transition={{ duration: 3, repeat: Infinity }}>
        <rect x="55" y="25" width="30" height="26" rx="3" fill="rgba(45,74,138,0.12)" stroke="rgba(45,74,138,0.4)" strokeWidth="0.5" />
        <path d="M62,25 L62,18 Q62,10 70,10 Q78,10 78,18 L78,25" fill="none" stroke="rgba(45,74,138,0.4)" strokeWidth="1.5" strokeLinecap="round" />
        <circle cx="70" cy="36" r="3" fill="#2D4A8A" opacity="0.6" />
        <line x1="70" y1="39" x2="70" y2="44" stroke="#2D4A8A" strokeWidth="1" opacity="0.6" />
      </motion.g>
      <motion.g animate={{ opacity: [0.3, 0.8, 0.3] }} transition={{ duration: 4, repeat: Infinity }}>
        <rect x="15" y="30" width="24" height="10" rx="2" fill="rgba(45,74,138,0.1)" stroke="rgba(45,74,138,0.3)" strokeWidth="0.5" />
        <text x="27" y="37" textAnchor="middle" fill="#2D4A8A" fontSize="4">OIDC</text>
      </motion.g>
      <motion.g animate={{ opacity: [0.5, 1, 0.5] }} transition={{ duration: 4, delay: 1, repeat: Infinity }}>
        <rect x="100" y="30" width="24" height="10" rx="2" fill="rgba(31,122,79,0.1)" stroke="rgba(31,122,79,0.3)" strokeWidth="0.5" />
        <text x="112" y="37" textAnchor="middle" fill="#1F7A4F" fontSize="4">JWT</text>
      </motion.g>
      {[0, 1, 2].map((i) => (
        <motion.rect
          key={i}
          ...
          stroke="rgba(45,74,138,0.2)"
          strokeWidth="0.3"
          animate={{ opacity: [0.3, 0.7, 0.3] }}
          transition={{ duration: 3, delay: i * 0.5, repeat: Infinity }}
        />
      ))}
      <text x="70" y="67" textAnchor="middle" fill="#6B6860" fontSize="3.5">MULTI-TENANT</text>
    </svg>
  );
}
...
function NotifyVisual() {
  // accent #FF5D1F, accent-2 #2D4A8A, success #1F7A4F
  const channels = [
    { icon: "WS", y: 20, color: "#FF5D1F" },
    { icon: "SMTP", y: 40, color: "#2D4A8A" },
    { icon: "FCM", y: 60, color: "#1F7A4F" },
  ];

  return (
    <svg viewBox="0 0 140 80" className="w-full h-full max-w-[200px]" aria-hidden="true">
      <rect x="10" y="28" width="28" height="24" rx="3" fill="rgba(255,93,31,0.1)" stroke="rgba(255,93,31,0.3)" strokeWidth="0.5" />
      <text x="24" y="38" textAnchor="middle" fill="#FF5D1F" fontSize="4" fontWeight="600">NATS</text>
      <text x="24" y="45" textAnchor="middle" fill="#6B6860" fontSize="3">EVENT</text>
      {channels.map((ch, i) => (
        <g key={ch.icon}>
